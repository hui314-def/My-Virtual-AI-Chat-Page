...
项目具有较高的自由度：你可以自由设定角色的名字、人设、头像和对话背景图，调整模型参数（温度、上下文长度等）以生成多样化的回复，可以让 AI 的回复通过语音合成朗读出来，并支持语音输入。打造属于你的专属对话角色人格。你还可以使用话题管理功能组织对话脉络，随时切换话题场景，并将精彩对话导出为 JSON 或 HTML 文件保存或分享。
...
- 💾 **数据持久化** — 聊天记录与设置自动保存在浏览器 IndexedDB 中，重启不丢失；支持接入 MySQL 后端实现多设备云同步。
...
默认监听端口 8001。启动前在项目根目录 `.env` 配置文件
