...
若需要 AI 生图功能，需安装 ComfyUI，并将工作流文件（`image_gen_workflow.json`）放置于项目目录下（示例工作流，可按需修改和替换）。随后启动图片生成 API：
...
- 文件上传：支持上传 `.txt`、`.md`、`.json` 等文本文件，可直接将文件拖入网页中，内容将自动附加到消息中发送给模型。
...
- 沉浸模式体验：默认快捷键ctrl+shift+f隐藏侧边栏和上边栏
