...
你可以自由设定角色的名字、人设、头像和背景图，调整模型参数（温度、上下文长度等），甚至让 AI 的回复通过语音合成朗读出来，并支持实时语音输入。

此外，你还可以使用话题管理功能组织对话脉络，随时切换话题场景，并将精彩对话导出为 JSON 或 HTML 文件保存或分享。
...
默认 Ollama API 地址为 `http://localhost:11434`，可在网页“个性化设置”中修改。

#### OpenAI 兼容 API

支持任何兼容 OpenAI 格式的 API（如 DeepSeek、智谱等），在设置中填写 API Key 和 Base URL（例如 `https://api.deepseek.com`）。
...
若需要 AI 生图功能，需安装 ComfyUI，并将工作流文件（`image_gen_workflow.json`）放置于项目目录下（示例工作流需自行配置）。随后启动图片生成 API：
...
- 语音合成和图片生成服务需额外配置，非必需功能。
- 图片生成使用 ComfyUI 工作流，需自行准备模型文件和工作流 JSON。
- 强烈建议在生产环境中启用 API Key 鉴权，避免未授权访问。
- 由于使用浏览器 IndexedDB 存储数据，请勿在多设备间共享同一浏览器数据，以免覆盖。
