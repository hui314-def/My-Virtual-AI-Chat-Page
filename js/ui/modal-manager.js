...
import { TTsService } from '../media/tts-service.js';
...
用户的风格描述：${style}`;
        ...
        if (topics.length === 0) {
            ...
            container.querySelectorAll('.topic-delete-btn').forEach(btn => {
                btn.addEventListener('click', async (e) => {
                    ...
                    // 删除确认：可选择「直接删除」或「提取话题记忆后删除」
                    const choice = await self.showCustomDialog({
                        title: `<i class="fas fa-trash-alt"></i> 删除话题 ${idx + 1}`,
                        message: '删除前是否先提取该话题中的记忆？',
                        buttons: [
                            { text: '取消', value: 'cancel', className: '' },
                            { text: '直接删除', value: 'delete', className: 'danger' },
                            { text: '提取记忆后删除', value: 'extract', className: 'save' },
                        ],
                    });
                    if (choice === 'cancel') return;
                    if (choice === 'extract') {
                        const topicToExtract = topics[idx];
                        if (topicToExtract) {
                            try {
                                await ctx.extractTopicMemory(ctx.currentChatId, topicToExtract.messages || []);
                                self.showBriefToast('🧠 话题记忆已提取，正在删除...');
                            } catch (err) {
                                console.warn('[Memory] 话题记忆提取失败:', err);
                                self.showBriefToast('⚠ 记忆提取失败，已取消删除');
                                return;   // 提取失败中止删除，避免信息丢失
                            }
                        }
                    }
    ...
    #renderKbSelectorList(container, kbList, selectedIds = []) {
        ...
        for (const kb of kbList) {
            const isMemory = !!(kb.is_memory || kb.id === '__memory__');
            const isSelected = isMemory || selectedIds.includes(kb.id);
            const desc = kb.description || '暂无描述';
            html += `
                <div class="kb-select-card ${isSelected ? 'selected' : ''}" data-kb-id="${kb.id}" ${isMemory ? 'data-locked="1"' : ''} style="${isMemory ? 'border-color:rgba(124,88,255,0.55); background:rgba(124,88,255,0.12);' : ''}">
                    <div class="kb-select-check"><i class="fas fa-check"></i></div>
                    <span class="kb-select-icon"><i class="${isMemory ? 'fas fa-brain' : 'fas fa-database'}" style="${isMemory ? 'color:#a78bfa;' : ''}"></i></span>
                    <div class="kb-select-name">${escapeHtml(kb.name)}</div>
                    <div class="kb-select-desc">${escapeHtml(desc)}${isMemory ? '<br><span style="color:#a78bfa; font-size:0.75rem;">🔒 始终启用（存放角色记忆）</span>' : ''}</div>
                </div>
            `;
        }
        html += `</div>`;
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                // 取消所有普通卡片选中;角色记忆库(locked)保持启用
                container.querySelectorAll('.kb-select-card').forEach(c => {
                    if (c.dataset.locked !== '1') c.classList.remove('selected');
                });
                this.#updateKbSelectorHint(container);
            });
        }
        // 绑定卡片点击事件(多选切换)
    ...
    /** 更新知识库选择提示(角色记忆库始终计入选中) */
    #updateKbSelectorHint(container) {
    ...
    confirmKnowledgeBaseSelection() {
        ...
        // 双保险:角色记忆库始终启用,即使 UI 状态异常也强制加入
        if (!ids.includes('__memory__')) {
            ids.unshift('__memory__');
            names.unshift('角色记忆库');
        }
