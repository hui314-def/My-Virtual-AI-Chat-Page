...
export class MemoryExtractor {
    ...
    async extractFromChat(chatId) {
        ...
        if (!text.trim()) return;
        await this.#extract(chatId, text);
        this._lastExtractCount.set(chatId, this.#countMessages(chat));
    }

    /** 按话题提取(删除话题时调用)：提取该话题全部消息的记忆 */
    async extractFromTopic(chatId, topicMessages) {
        if (!this.getIsMemoryEnabled()) return;
        if (!Array.isArray(topicMessages) || topicMessages.length === 0) return;
        const text = this.#collectRecentText({ topics: [{ messages: topicMessages }] });
        if (!text.trim()) return;
        await this.#extract(chatId, text);
    }
