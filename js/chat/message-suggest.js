...
export class MessageSuggest {
     ...
     * @param {Function} ctx.getModalEl — () => #suggest-modal 弹窗(惰性获取,弹窗模板运行时注入)
    ...
    constructor(ctx) {
        ...
        this.getModalEl = ctx.getModalEl;
    ...
    /** 惰性获取 #suggest-modal 弹窗元素(模板在 init() 中注入,打开弹窗时必已存在) */
    get modalEl() {
        return this.getModalEl() || null;
    }

    /** 惰性获取建议内容容器 #suggest-body */
    get _bodyEl() {
        return this.modalEl ? this.modalEl.querySelector('#suggest-body') : null;
    }
